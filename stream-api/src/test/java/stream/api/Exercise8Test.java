...
public class Exercise8Test extends ClassicOnlineStore {
    ...
    public void havingEnoughMoney() {
        ...
        List<Item> onSale = shopStream.flatMap(shop -> shop.getItemList().stream())
            ...
            .collect(Collectors.toList());
        Predicate<Customer> havingEnoughMoney = customer -> customer.getBudget() > customer
            .getWantToBuy()
            .stream()
            .map(item -> onSale.stream().filter(saleItem -> saleItem.getName().equals(item.getName())).mapToInt(Item::getPrice).min().orElse(0))
